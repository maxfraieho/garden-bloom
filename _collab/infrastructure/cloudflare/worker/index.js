...
async function fetchNotebookLM(env, path, init = {}) {
  const baseUrl = (env.NOTEBOOKLM_BASE_URL || 'https://notebooklm.exodus.pp.ua').replace(/\/$/, '');
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // ============================================
      // MEMORY API PROXY (all /v1/memory/* → backend via nginx)
      // ============================================
      if (path.startsWith('/v1/memory')) {
        ...
        const memoryRes = await fetchNotebookLM(env, path + url.search, {
          method,
          headers: Object.fromEntries(request.headers),
          body: method !== 'GET' && method !== 'HEAD' ? await request.text() : undefined,
        });
        // Forward response as-is with CORS
        return jsonResponse(memoryRes.data, memoryRes.status);
      }
