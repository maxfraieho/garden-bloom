...
- [[ARCHITECTURE_ROOT]] — аксіоми A3 (stateless: агент = _agent.md, не стан runtime), A7 (bounded memory)

**Цей документ залежить від:**
- [[AGENT_MEMORY_GIT_DIFFMEM_V1]] — модель пам'яті агента (Layer 1/2, HARD limits)
- [[AGENT_LOGIC_VERSIONING_V1]] — версіонування logic/ (logic-update proposals)
- [[DRAKON_ІНТЕГРАЦІЯ_ТА_МОДЕЛЬ_ВИКОНАННЯ_АГЕНТА]] — DRAKON → pseudocode → _agent.md

**Від цього документа залежать:**
- [[RUNTIME_ARCHITECTURE_CANONICAL]] — Mastra інтерпретує цей контракт
- [[EXECUTION_PIPELINE_CANONICAL]] — Phase 3 (Context Load): завантаження _agent.md та memory
