...
**[ПРИНЦИП]** Архітектура описує ролі та контракти, не vendor-specific реалізації. Orchestration Layer — замінний компонент (vendor-agnostic; adapter-based). Конкретний оркестратор обирається пізніше (Trigger.dev / Temporal / BullMQ / …). Деталі абстракції — у ORCHESTRATION_LAYER_ABSTRACTION.md.
...
| 2 | **MinIO = Source of Truth** — canonical storage для всіх агентних даних | STORAGE_AUTHORITY_MODEL_CANONICAL.md §1 |
...
|----------|------|--------------|
| **MinIO** | Canonical storage | STORAGE_AUTHORITY_MODEL_CANONICAL.md |
