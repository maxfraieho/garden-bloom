...
Memory API забезпечує доступ до DiffMem — git-backed системи пам'яті агентів.

**Base path:** `/v1/memory`
**Proxy:** CF Worker → Nginx → Memory API (:3001)
**Auth:** Owner JWT (усі endpoints крім health)
**Implementation details:** див. `docs/memory/API_CONTRACT.md`

### 10.0 Context Depth Modes

Всі context endpoints підтримують 4 рівні глибини:

| Depth | Опис | Token cost |
|-------|------|------------|
| `surface` | ALWAYS_LOAD блоки з top entities | ~500–1K |
| `wide` | BM25 search + ALWAYS_LOAD (default) | ~2–5K |
| `deep` | Full entity content | ~5–15K |
| `temporal` | Full content + git history diffs | ~10–30K |

> **Примітка:** Деякі implementation docs використовують `basic` замість `surface` — це еквівалентні терміни.

### 10.1 GET /v1/memory/search

BM25 пошук по current-state entities.
...
| `limit` | number | 10 | Кількість результатів |
...
{
  "results": [
    { "id": "people/alice", "title": "Alice", "score": 0.85, "snippet": "..." }
  ]
}
```
...
### 10.2 POST /v1/memory/context

Context assembly з depth mode.

**Request:**

```json
{
  "query": "garden bloom architecture",
  "depth": "wide",
  "maxDepth": 3,
  "maxTokens": 5000,
  "historyLimit": 10
...
`temporalAttachments` присутній **тільки** для `depth=temporal`.

---

### 10.3 GET /v1/memory/entities/:id

Читання entity (present-state). **Повертає entity напряму, не `{ entity }`.**
...
### 10.4 GET /v1/memory/entities/:id/history

Git commit history для entity.

**Query params:**

| Param | Тип | Default | Опис |
|-------|-----|---------|------|
| `limit` | number | 20 | Кількість комітів |
...
```json
{
  ...
  "history": [
    { "sha": "a1b2c3d", "timestamp": "2026-03-05T12:00:00Z", "message": "memory: session-010", "author": "agent" }
  ],
  "count": 12
}
```
...
### 10.5 GET /v1/memory/entities/:id/diff

Diff між ревізіями entity.

**Query params:**

| Param | Тип | Опис |
|-------|-----|------|
| `to` | string | SHA цільової ревізії (обов'язковий) |
| `from` | string | SHA базової ревізії (optional, default: parent of `to`) |
...
### 10.6 GET /v1/memory/entities/:id/revisions/:sha

Read-only snapshot entity у конкретній ревізії.
...
### 10.7 POST /v1/memory/:userId/process-transcript

Обробка тексту, витягнення entities.

**Request:**

```json
{
  "memoryInput": "Had a meeting with Alice about roadmap...",
  "sessionId": "session-2026-03-08",
  "autoCommit": true
}
```
...
### 10.8 GET /v1/memory/health

Health check (без автентифікації).
...
```json
{ "sha": "abc123", "message": "manual commit" }
```
...
- **frontend/ПАМ_ЯТЬ_DIFFMEM_ІНТЕГРАЦІЯ.md** — frontend DiffMem integration spec

---

*Цей документ є канонічною специфікацією API контрактів системи Garden Bloom.*
