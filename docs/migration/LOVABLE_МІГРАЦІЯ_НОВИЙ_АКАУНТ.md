...
|---------|---------|------|
| CORS error у консолі | Lovable preview URL не в ALLOWED_ORIGINS Worker | Додати URL у Worker або тимчасово `Access-Control-Allow-Origin: *` |
| 401 Unauthorized на /health | Worker вимагає auth для /health | /health має бути публічним — перевірити Worker код |
| 404 на API calls | Неправильний `DEFAULT_GATEWAY` URL | Оновити `mcpGatewayClient.ts:14` |
| Trailing slash | `https://worker.dev/` замість `https://worker.dev` | Видалити trailing `/` з URL |
| Білий екран | Build error або відсутній файл | Перевірити Console → Network, перезапустити preview |
| "fetch failed" | Worker не відповідає або DNS не резолвиться | Перевірити Worker deployment у Cloudflare Dashboard |
| ERR_ABORTED preflight | Відсутній `X-Correlation-Id` в дозволених заголовках | Додати `X-Correlation-Id` до `Access-Control-Allow-Headers` у Worker |

---

## 6. Після міграції

- [ ] Перевірити що Owner Login працює (якщо був налаштований)
- [ ] Перевірити Access Zones (якщо використовуються)
...
Якщо щось зламалось — повернути `DEFAULT_GATEWAY` у `mcpGatewayClient.ts:14` на старе значення:
