...
import { Lock, ArrowLeft, GitBranch, Plus, Save } from 'lucide-react';
...
export default function DrakonPage() {
  ...
  const handleStartEdit = () => {
    if (!autoSlug || !selectedFolder) return;
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 px-4 py-4">
          ...
          {step === 'select' ? (
            /* Creation form — same layout as article editor */
            <div className="flex flex-col gap-4 max-w-3xl">
              {/* Diagram name input */}
              <Input
                value={diagramName}
                onChange={(e) => setDiagramName(e.target.value)}
                placeholder={t.drakonEditor.diagramNamePlaceholder || 'Diagram name...'}
                className="text-xl font-semibold h-11 px-3 border-border/50 focus-visible:border-primary bg-background"
                autoFocus
              />
