---
{"tags":["domain:agent","status:canonical","format:contract"],"created":"2026-02-21","updated":"2026-02-21","tier":1,"title":"КОНТРАКТ_АГЕНТА_V1","dg-publish":true,"dg-metatags":null,"dg-home":null,"permalink":"/exodus.pp.ua/architecture/КОНТРАКТ_АГЕНТА_V1/","dgPassFrontmatter":true,"noteIcon":""}
---

# Контракт агента V1

> Створено: 2026-02-14
> Оновлено: 2026-02-21 (v1.1 — оновлено модель пам'яті відповідно до AGENT_MEMORY_GIT_DIFFMEM_V1.md v1.1.0)
> Автор: Архітектор системи
> Базується на: RUNTIME_ARCHITECTURE_CANONICAL.md, АРХІТЕКТУРНА_БАЗА_СИСТЕМИ.md, MANIFESTO.md, AGENT_MEMORY_GIT_DIFFMEM_V1.md
...
└── agents/
    └── <slug>/                     ← унікальний ідентифікатор агента
        ...
        │                           (деталі: AGENT_MEMORY_GIT_DIFFMEM_V1.md)
        │
...
> Повний опис структури пам'яті — AGENT_MEMORY_GIT_DIFFMEM_V1.md
...
*Цей документ є специфікацією контракту агента V1. Зміни до контракту вимагають оновлення версії документа та узгодження з RUNTIME_ARCHITECTURE_CANONICAL.md.*
...
- [[exodus.pp.ua/architecture/ARCHITECTURE_ROOT\|ARCHITECTURE_ROOT]] — аксіоми A3 (stateless: агент = _agent.md, не стан runtime), A7 (bounded memory)

**Цей документ залежить від:**
- [[exodus.pp.ua/architecture/AGENT_MEMORY_GIT_DIFFMEM_V1\|AGENT_MEMORY_GIT_DIFFMEM_V1]] — модель пам'яті агента (Layer 1/2, HARD limits)
- [[exodus.pp.ua/architecture/AGENT_LOGIC_VERSIONING_V1\|AGENT_LOGIC_VERSIONING_V1]] — версіонування logic/ (logic-update proposals)
- [[exodus.pp.ua/architecture/DRAKON_ІНТЕГРАЦІЯ_ТА_МОДЕЛЬ_ВИКОНАННЯ_АГЕНТА\|DRAKON_ІНТЕГРАЦІЯ_ТА_МОДЕЛЬ_ВИКОНАННЯ_АГЕНТА]] — DRAKON → pseudocode → _agent.md

**Від цього документа залежать:**
- [[exodus.pp.ua/architecture/RUNTIME_ARCHITECTURE_CANONICAL\|RUNTIME_ARCHITECTURE_CANONICAL]] — Mastra інтерпретує цей контракт
- [[exodus.pp.ua/architecture/EXECUTION_PIPELINE_CANONICAL\|EXECUTION_PIPELINE_CANONICAL]] — Phase 3 (Context Load): завантаження _agent.md та memory
- [[exodus.pp.ua/architecture/INBOX_ТА_PROPOSAL_АРХІТЕКТУРА\|INBOX_ТА_PROPOSAL_АРХІТЕКТУРА]] — агент породжує proposals як safe output
