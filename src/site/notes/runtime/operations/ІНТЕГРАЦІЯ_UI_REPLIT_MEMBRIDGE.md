...
created: 2026-02-25
updated: 2026-02-27
legacy_name: "ІНТЕГРАЦІЯ_UI_REPLIT_MEMBRIDGE.md"
changelog:
  - 2026-02-27 (rev 4): Замінено "у Replit" → "коли Membridge offline". Production stack — Alpine + Lovable frontend. 502 — нормальна поведінка при недоступному Membridge, а не специфіка платформи.
...
title: "Інтеграція UI Replit з Membridge"
dg-publish: true
---
...
- Якщо Membridge **недоступний** → `502 {"error": "fetch failed"}` (нормальна поведінка коли Membridge offline)
...
### Що є нормою коли Membridge offline

Якщо Membridge control plane **не запущений**, тому:
- `GET /api/membridge/*` → `502 {"error": "fetch failed"}` — **це нормально**
- Runtime endpoints (`/api/runtime/*`) працюють повноцінно з PostgreSQL
- У production (Alpine): Membridge запущений на `:8000`, `502` не має виникати
...
- [[СТАН_РОЗГОРТАННЯ_RUNTIME_ALPINE.md]] — топологія розгортання
- [[СТАН_РЕАЛІЗАЦІЇ_BACKEND_RUNTIME.md]] — стан бекенду

**На цей документ посилаються:**
- [[ПРОГАЛИНИ_ТА_НАСТУПНІ_КРОКИ_RUNTIME.md]] — GAP-7 (позначений як RESOLVED)
