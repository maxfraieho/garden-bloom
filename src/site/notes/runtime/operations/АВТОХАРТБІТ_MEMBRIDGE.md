...
## Auto-sync ключів (НОВЕ — 2026-02-28)

При реєстрації з Runtime (`POST /api/runtime/workers`), агент отримує
актуальні ключі і автоматично оновлює `.env.agent`:

| Ключ | Опис |
|------|------|
| `MEMBRIDGE_AGENT_KEY` | Аутентифікація агента (X-MEMBRIDGE-AGENT) |
| `MEMBRIDGE_ADMIN_KEY` | Аутентифікація heartbeat до CP (X-MEMBRIDGE-ADMIN) |

Це усуває необхідність ручного копіювання ключів між нодами.
Ключі оновлюються лише при зміні — `.env.agent` не перезаписується зайво.
...
- Keys are auto-synced from Runtime — manual configuration only needed for initial setup.
