...
created: 2026-02-22
updated: 2026-03-15
legacy_name: "API_CONTRACT.md"
title: "Контракт API пам'яті"
tier: 2
---

# Контракт API пам'яті v1

> REST API specification for the DiffMem-like memory subsystem.
> **Примітка:** Canonical source of truth для API contracts — [`backend/КОНТРАКТИ_API_V1.md`](../backend/КОНТРАКТИ_API_V1.md) §10. Цей файл є implementation-level деталізацією.
...
| Depth | Alias | Behavior |
|-------|-------|----------|
| `surface` | `basic` | ALWAYS_LOAD blocks from top-ranked entities |
| `wide` | — | BM25 search over conversation + ALWAYS_LOAD (default) |
| `deep` | — | Full file content of matched entities |
| `temporal` | — | Full files + git diffs for last N commits; response includes `temporalAttachments` |

> **Note:** `surface` та `basic` є синонімами. Canonical термін — `surface` (типи frontend). Деякі backend реалізації використовують `basic`.
