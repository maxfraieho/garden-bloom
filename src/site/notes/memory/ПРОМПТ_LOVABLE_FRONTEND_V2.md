...
> **⚠️ PROMPT ARTIFACT.** Це не canonical spec — це промпт для Lovable Agent. Актуальна специфікація: [`frontend/ПАМ_ЯТЬ_DIFFMEM_ІНТЕГРАЦІЯ.md`](../frontend/ПАМ_ЯТЬ_DIFFMEM_ІНТЕГРАЦІЯ.md).
...
| `docs/memory/АРХІТЕКТУРА_ПАМЯТІ.md` | Схема системи, DiffMem концепція, 4-depth context model |
