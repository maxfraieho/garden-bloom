...
│  ВИКОНАННЯ  СХОВИЩЕ  МУТАЦІЯ  КОНВЕЄР  ЦИКЛ  ОРКЕСТРАЦІЯ     │
│  КОНТРАКТ_АГЕНТА_V1                                           │
├─────────────────────────────────────────────────────────────┤
│  FEATURES                        architecture/features/       │
│  ПАМ_ЯТЬ_АГЕНТА  ВЕРСІОНУВАННЯ  DRAKON  ADR_КОМЕНТАРІВ       │
├─────────────────────────────────────────────────────────────┤
│  NON-FUNCTIONAL                  architecture/non-functional/ │
│  БЕЗПЕКА  ПРОДУКТИВНІСТЬ  МАСШТАБУВАННЯ  НАДІЙНІСТЬ            │
│  СПОСТЕРЕЖУВАНІСТЬ  ВАРТІСТЬ                                  │
├─────────────────────────────────────────────────────────────┤
...
├─────────────────────────────────────────────────────────────┤
│  RUNTIME                         runtime/                     │
│  ПОСІБНИК_КОРИСТУВАЧА  НАЛАШТУВАННЯ  RUNBOOK_ОНОВЛЕННЯ        │
│  operations/ (17 operational guides)                          │
├─────────────────────────────────────────────────────────────┤
│  INTEGRATION                     integration/                 │
│  MEMBRIDGE  FRONTEND_LOVABLE  MEMORY_BACKEND  NOTEBOOKLM      │
├─────────────────────────────────────────────────────────────┤
│  BOOTSTRAP                       bootstrap/                   │
│  README  profiles  supported-systems  architecture            │
├─────────────────────────────────────────────────────────────┤
│  WORKFLOW                        workflow/                     │
│  canonical-node-state  context-routing  skill-routing          │
├─────────────────────────────────────────────────────────────┤
│  AGENTS | DESIGN | PRODUCT | AUDIT | MIGRATION                │
├─────────────────────────────────────────────────────────────┤
...
### 2.8 Runtime — операційні runbooks

Документи для **операторів BLOOM runtime** та membridge.

| Документ | Що містить |
|----------|-----------|
| [[ПОСІБНИК_КОРИСТУВАЧА_BLOOM_RUNTIME]] | Загальний посібник користувача |
| [[НАЛАШТУВАННЯ_ТА_РОЗГОРТАННЯ]] | Розгортання runtime |
| [[RUNBOOK_ОНОВЛЕННЯ_ПІСЛЯ_GIT_PULL]] | Runbook оновлення після git pull |
| `runtime/operations/` | 17 операційних гайдів (membridge, scaling, deployment) |

### 2.9 Integration — інтеграційний шар

Специфікації **інтеграцій між компонентами** системи.

| Документ | Що містить |
|----------|-----------|
| [[ІНТЕГРАЦІЯ_MEMBRIDGE]] | Інтеграція membridge |
| [[ІНТЕГРАЦІЯ_FRONTEND_LOVABLE]] | Інтеграція з Lovable frontend |
| [[ІНТЕГРАЦІЯ_MEMORY_BACKEND]] | Інтеграція memory backend |
| [[ІНТЕГРАЦІЯ_NOTEBOOKLM_BACKEND]] | Інтеграція NotebookLM backend |
| [[INTEGRATION_BUNDLE_V1]] | Integration bundle v1 |
| `integration/` | 10 файлів загалом |

### 2.10 Bootstrap — bloom-node-bootstrap

Система автоматичного розгортання нових нод.

| Документ | Що містить |
|----------|-----------|
| [[bootstrap/README]] | Overview системи bootstrap |
| [[bootstrap/profiles]] | Профілі встановлення |
| [[bootstrap/supported-systems]] | Підтримувані системи |
| [[bootstrap/architecture]] | Архітектура bootstrap |
| [[bootstrap/migration]] | Міграція між версіями |

### 2.11 Workflow — claude-codex-skill ctx/chub/cgc

Специфікації **навичок Claude** для context routing та skill management.

| Документ | Що містить |
|----------|-----------|
| [[canonical-node-state]] | Canonical node state |
| [[context-routing]] | Context routing rules |
...
### 2.12 Agents — реєстр агентів

| Документ | Що містить |
|----------|-----------|
| [[agents/README]] | Структура та поля агентів |
| [[architect-guardian]] | Architect guardian agent |

### 2.13 Design — дизайн-система

| Документ | Що містить |
|----------|-----------|
| [[BLOOM_DESIGN_SYSTEM]] | Design system Garden Bloom |

### 2.14 Historical — архівні матеріали
...
АРХІТЕКТУРНИЙ_КОРІНЬ
  ...
  → ПАМ_ЯТЬ_DIFFMEM_ІНТЕГРАЦІЯ     ← DiffMem frontend integration
```
...
### Маршрут F — Runtime / Membridge оператор
```
ПОСІБНИК_КОРИСТУВАЧА_BLOOM_RUNTIME
  → НАЛАШТУВАННЯ_ТА_РОЗГОРТАННЯ
  → RUNBOOK_ОНОВЛЕННЯ_ПІСЛЯ_GIT_PULL
  → integration/_INDEX.md
  → BLOOM_RUNTIME_NODE_CAPABILITIES_V1
  → BLOOM_RUNTIME_FAILURE_MODEL_V1
```

### Маршрут G — Bootstrap / нова нода
```
bootstrap/README
  → bootstrap/profiles
  → bootstrap/supported-systems
  → bootstrap/architecture
```

### Маршрут H — ctx/chub/cgc workflow
```
workflow/canonical-node-state
